class DiscussionBroadcaster
  ...
  def broadcast!
    replace_header
    replace_post_form if discussion.saved_change_to_closed?
  ...
  def move_categories
    ...
    move_discussion_from_category(old_category, new_category)
    update_categories(old_category, new_category)
  end

  # broadcast remove discussion from old category list / insert it to new category list

  def move_discussion_from_category(old_category, new_category)
